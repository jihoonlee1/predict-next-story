...
def prepare_data(cur, roots):
	...
	for root_id, company_id, root_content in roots:
		root_content = clean_content(root_content)
		...
		for child_content, in cur.fetchall():
			child_content = clean_content(child_content)
			sent0.append(root_content)
			sent1.append(child_content)
			labels.append([1, 0])

		cur.execute("SELECT content FROM root_children_positive1 WHERE root_id = ? and company_id = ?", (root_id, company_id))
		for child_content, in cur.fetchall():
			child_content = clean_content(child_content)
			sent0.append(root_content)
			sent1.append(child_content)
